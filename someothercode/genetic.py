...
class gene(object):
    ...
    def __init__(self):
        ...
        #self.pos_1 = (0,0)
        #self.pos_2 = (0,0)
        # self.pos_2 = random.randint(0, 500)
        self.radius= 0
        #self.color
        # self.radius=1
        # #self.color = {'r': 255,
    ...
    def mutate(self):
        ...
        mutation_size = max(1, int(round(random.gauss(40, 4)))) / 100
        self.radius=max(self.radius,random.randint(int(self.radius * (1 - mutation_size)), min(int(self.radius * (1 + mutation_size)),500 // 5)))

        self.pos_1 = (self.x-self.radius,self.y-self.radius)
        self.pos_2 = (self.x+self.radius,self.y+self.radius)
        #self.radius= random.randint(0, 500)//8
        #self.radius=max(self.radius,random.randint(int(self.radius * (1 - mutation_size)), min(int(self.radius * (1 + mutation_size)),500 // 5)))
